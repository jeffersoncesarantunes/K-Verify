...
static void print_progress_bar(int value, int max, int width)
{
    ...
    for (int i = 0; i < width; i++) {
        if (i < filled)
            printf(CLR_BOLD_GREEN "█" CLR_RESET);
        else
            printf("░");
    }
}
...
void export_results_terminal(ScenarioResult *results, int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        printf("%s%-12s" CLR_RESET,
               ...
               r->detection.kscanner_detected ? "● detect" : "○ miss");
        printf(" %s%s" CLR_RESET,
               r->detection.linspec_detected ? CLR_RED : CLR_GREEN,
               r->detection.linspec_detected ? "● detect" : "○ miss");
        printf("\n");
    }
    ...
    for (int i = 0; i < count; i++) {
        switch (results[i].execution_result) {
            case RESULT_PASS:  printf(CLR_GREEN "●" CLR_RESET); break;
            case RESULT_FAIL:  printf(CLR_RED "○" CLR_RESET); break;
            case RESULT_WARN:  printf(CLR_YELLOW "◐" CLR_RESET); break;
            case RESULT_SKIP:  printf(CLR_CYAN "○" CLR_RESET); break;
    ...
    printf("\n");
    printf("  " CLR_GREEN "●" CLR_RESET " Validation complete.\n");
}
